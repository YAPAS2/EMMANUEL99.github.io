...
class Pi_WooCommerce_Quick_Buy_Auto_Add {
	...
	public function add_quick_buy_button() {
		...
		if(!$this->productLevelDisable($product_id)){
			...
			*/
			...
			echo '<button class="button pisol_single_buy_now pisol_buy_now_button '.$class.' '.$custom_buy_now_class.' " type="submit" name="add-to-cart" value="'.esc_attr($product_id).'">'.esc_html($this->label_product).'</button>';
		}
	}
	...
	static function getFirstVariation($product){
		...
		$first_variation = $all_variations[0];
	...
	public function get_variableproduct_addtocartLink($parent_id, $variation_id,$attributes, $qty = 1 , $page= 'checkout') {
		if ( $variation_id != 0 ) {
			...
			$link = $checkout.'?add-to-cart='.$parent_id.'&pi_quick_checkout=1&variation_id='.$variation_id.$attributes;
			return $link;
		}
		return false;
	}
