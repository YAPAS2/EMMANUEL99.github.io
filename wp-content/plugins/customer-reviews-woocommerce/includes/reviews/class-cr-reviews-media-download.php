	...
	class CR_Reviews_Media_Download {
		...
		public function download_media_frontend() {
			...
			if( !$transientFrontendExists && isset( $_POST['reviewID'] ) && 0 < intval( $_POST['reviewID'] ) ) {
				...
				$mediaFiles = $wpdb->get_results( "SELECT comment_id, meta_key, meta_value FROM $wpdb->commentmeta WHERE comment_id = '" . $reviewID . "' AND meta_key = 'ivole_review_image'", OBJECT );

				$mediaFileToDownload = null;
				$downloadURL = '';
				$commentMeta = array( 'newCache' => 'cr_media_cache' );
				if( $mediaFiles ) {
					error_log( print_r($mediaFiles,true) );
